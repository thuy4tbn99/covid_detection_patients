...
def extract_basic_info(text_block):
    regex_dict = {
        ...
        "maBN":"(BN\s?[0-9]+)",
        "namSinh":"(((Sinh năm)|(sinh năm)|(SN)|(Sinh ngày))[:]*.?[\s\d\/]*\d{4})",
...
def export_to_excel(patient_infos, publish_date, ofile_path):
    ...
    cols = [
        'Ngày công bố', 'MCB', 'Tên BN', 'Năm sinh', 'Tuổi', 'Nhóm tuổi',
        ...
        'Số điện thoại [bệnh nhân]', 'Tóm tắt dịch tễ/ Ghi chú', 'CMND', 'Ca F0 liên quan', 'file_path'
    ]
